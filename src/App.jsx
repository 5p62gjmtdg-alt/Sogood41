...
function cleanAddress(displayName) {
  ...
  // Cherche: numéro+rue, puis code postal+ville
  const streetPart = parts[0]; // ex: "14 Rue Roger Leclerc"
  const zipCity = parts.find(p=>/\b\d{5}\b/.test(p)); // ex: "41000 Blois"
  if (zipCity) return `${streetPart}, ${zipCity}`;
  // Fallback: prend les 2 premiers segments significatifs
...
// Basé sur: distance camion→prochain client + stops avant moi
function estimateWait(myPos, waiting, truckPos, myLat, myLng) {
  if (myPos <= 0) return null;
  const stopsBeforeMe = myPos - 1;
  let totalMins = stopsBeforeMe * MINS_PER_STOP;
  // Si on a la position du camion ET mon adresse, on ajoute le temps de trajet
  if (truckPos && myLat && myLng) {
    const kmToMe = haversineKm(truckPos[0], truckPos[1], myLat, myLng);
    // ~30km/h en ville = 2 min par km
    const driveToMe = Math.round(kmToMe * 2);
    totalMins = stopsBeforeMe * MINS_PER_STOP + driveToMe;
  }
...
function ClientView({ onSecretTap }) {
  ...
  function validate(){
    ...
    if(Object.keys(form.order).length===0) e.order="Choisis au moins un produit";
  ...
  async function handleSubmit(){
    ...
    const orderArr=Object.entries(form.order).flatMap(([id,qty])=>Array(qty).fill(id));
    const entry={id,...form,order:orderArr,lat:selAddr?.lat,lng:selAddr?.lng,
  ...
  function addItem(id){setForm(f=>({...f,order:{...f.order,[id]:(f.order[id]||0)+1}}));}
  function removeItem(id){setForm(f=>{
    const o={...f.order};
    if(o[id]>1) o[id]--;
    else delete o[id];
    return {...f,order:o};
  ...
  const waitMins=estimateWait(myPos, waiting, truck, me?.lat, me?.lng);
  ...
  if(step==="preview") return (
    <div>
      ...
      <div style={{...S.card,marginTop:12,background:"linear-gradient(135deg,#FFD600,#ffe94d)",border:"none"}}>
        <p style={{color:"#111",fontSize:11,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:2,margin:"0 0 4px"}}>
          TEMPS D'ATTENTE ESTIMÉ</p>
        {waiting.length===0?(
          <p style={{color:"#111",fontWeight:700,fontSize:18,margin:0}}>🎉 Aucune attente — commande maintenant !</p>
        ):(
          <>
            <div style={{display:"flex",alignItems:"baseline",gap:6}}>
              <span style={{fontSize:64,fontFamily:"'Bebas Neue',sans-serif",color:"#111",lineHeight:1}}>
                ~{waiting.length * MINS_PER_STOP}</span>
              <span style={{color:"rgba(0,0,0,.5)",fontSize:20}}>min</span>
            </div>
            <p style={{color:"#111",fontSize:13,fontWeight:600,margin:"4px 0 0"}}>
              {waiting.length} livraison{waiting.length>1?"s":""} en cours avant toi</p>
          </>
        )}
      </div>
          ...
          <div key={cat} style={{marginBottom:12}}>
            ...
            {stock.filter(s=>s.cat===cat).map(item=>(
              <div key={item.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                ...
                border:item.qty===0?"1.5px solid #3a0000":form.order[item.id]?"1.5px solid #FFD600":"1.5px solid #2a2a2a",
                background:item.qty===0?"#1a0000":form.order[item.id]?"#2a2200":"#222"}}>
                <div>
                  <span style={{color:item.qty===0?"#555":"#fff",fontSize:14,
                    textDecoration:item.qty===0?"line-through":"none"}}>{item.name}</span>
                  {item.qty>0&&<span style={{color:"#FFD600",fontFamily:"'Bebas Neue',sans-serif",fontSize:13,marginLeft:8}}>{item.price}€</span>}
                </div>
                {item.qty===0
                  ?<span style={{fontSize:11,fontWeight:700,color:"#f55",background:"#2a0000",
                    padding:"2px 8px",borderRadius:8,border:"1px solid #5a0000",whiteSpace:"nowrap"}}>🚫 Indisponible</span>
                  :<div style={{display:"flex",alignItems:"center",gap:8}}>
                    {form.order[item.id]&&<button onClick={()=>removeItem(item.id)}
                      style={{width:28,height:28,borderRadius:"50%",background:"#333",border:"none",
                        color:"#fff",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>}
                    {form.order[item.id]&&<span style={{color:"#FFD600",fontFamily:"'Bebas Neue',sans-serif",
                      fontSize:18,minWidth:20,textAlign:"center"}}>{form.order[item.id]}</span>}
                    <button onClick={()=>addItem(item.id)}
                      style={{width:28,height:28,borderRadius:"50%",background:"#FFD600",border:"none",
                        color:"#111",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                  </div>
                }
              </div>
            ))}
        ...
        {form.order.length>0&&(
          <div style={{background:"#1a1a00",borderRadius:10,padding:"10px 12px",marginBottom:14}}>
            ...
            <p style={{color:"#fff",fontWeight:700,fontSize:20,margin:0}}>
              {Object.entries(form.order).reduce((s,[id,qty])=>s+(stock.find(x=>x.id===id)?.price||0)*qty,0)}€</p>
          </div>
        )}
